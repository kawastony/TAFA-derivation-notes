# TAFA derivation notes

Working notes only. These are **not** papers.

The paper stack stays in [`Quantum_Gravity`](https://github.com/kawastony/Quantum_Gravity) and [`Quadratic-Mechanism-Lens`](https://github.com/kawastony/Quadratic-Mechanism-Lens). This repository exists so the closure work can be read without being mixed into that stack.

## What is here

| File | What it is |
|---|---|
| [`notes/Forced_vs_Free_TAFA_bridges.md`](notes/Forced_vs_Free_TAFA_bridges.md) | Closure note: axioms, forced lemmas, named open theorems |
| [`notes/TAFA_stress_tensor_and_pressure.md`](notes/TAFA_stress_tensor_and_pressure.md) | Law 4 as \(T_{\mu\nu}\): cosmological \(p\), quantum pressure \(p_Q\), wall, warp |

## Status

Research notes, 3 September 2026. Not a claim of UV completion.

Lineage (papers, elsewhere):

- Observer_Catalyst
- Quadratic-Mechanism-Lens (TIFA)
- Quantum_Gravity (TAFA papers)

## Licence

MIT, same as the other kawastony physics repos.
